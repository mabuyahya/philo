...
/*   Updated: 2025/04/08 15:22:14 by mabuyahy         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "philo.h"

int	init_mutexes_allocation(pthread_mutex_t **forks, int num, t_main *main)
{
	*forks = malloc(sizeof(pthread_mutex_t) * num);
	...
	if (!main->time_of_last_meal_mutex || !main->waiting_mutex
		|| !main->i_am_dead_mutex || !main->printf_mutex || !main->meals_mutex)
		return (1);
	return (0);
}

int	init_mutexes(pthread_mutex_t **forks, int num, t_main *main)
{
	int	i;
...
{
	if (init_mutexes_allocation(forks, num, main))
		return (1);
	if (init_mutexes(forks, num, main))
		return (1);
	return (0);
}
...
int	main_init(t_main *main, char **argv)
{
	...
	if (!main->philos_ids || !main->monitors_ids)
		return (printf_return_free(main, "malloc failed3"));
	return (0);
}
